	...
	: CDialog(CDemoDlg::IDD, pParent)
{
	...
	if (!SUCCEEDED(m_pRecorset.CreateInstance(__uuidof(Recordset))))
	{
		m_pRecorset = NULL;
...
void CDemoDlg::OnExecute() 
{
	...
	try
	{
		m_pRecorset = m_pCommand->Execute(&vNULL, &vNULL, adCmdText);
	}
	catch (_com_error& e)
	{
		TRACE(_T("%s\n"), e.ErrorMessage());
	...
	//读取记录集
	if (m_pRecorset->BOF && m_pRecorset->adoEOF)
	{
		return;
	}
	m_pRecorset->MoveFirst();
	int n = 0;
	while (!m_pRecorset->adoEOF)
	{
		_variant_t varValue;
		CString strName = _T("");
		int nAge = 0;

		varValue = m_pRecorset->GetCollect(_variant_t(_T("NAME")));
		...
		varValue = m_pRecorset->GetCollect(_variant_t(_T("AGE")));
		if (varValue.vt != VT_NULL)
		{
